...
def dashboard():
    if 'loggedin' in session:
        ...
        total_books_query = "SELECT COUNT(*) AS total_books FROM book"
        g.cursor.execute(total_books_query)
        total_books = g.cursor.fetchone()['total_books']

 
        available_books_query = "SELECT COUNT(*) AS available_books FROM book WHERE status = 'Enable'"
        g.cursor.execute(available_books_query)
        available_books = g.cursor.fetchone()['available_books']

 
        returned_books_query = "SELECT COUNT(*) AS returned_books FROM issued_book WHERE status = 'Returned'"
        g.cursor.execute(returned_books_query)
        returned_books = g.cursor.fetchone()['returned_books']


        issued_books_query = "SELECT COUNT(*) AS issued_books FROM issued_book WHERE status = 'Issued'"
        g.cursor.execute(issued_books_query)
        issued_books = g.cursor.fetchone()['issued_books']

 
        g.cursor.close()

        return render_template("dashboard.html", total_books=total_books, available_books=available_books,
                               returned_books=returned_books, issued_books=issued_books)
    ...
    if 'loggedin' in session:        
        ...
        if request.method == 'POST' and 'role' in request.form and 'first_name' in request.form and 'last_name' in request.form and 'email' in request.form :
            ...
            if action == 'updateUser':
                ...
                if userId.isdigit():  # Check if userId is a valid numeric string
                    userId = int(userId)                 
...
def register():
    ...
    if request.method == 'POST' and 'first_name' in request.form and 'last_name' in request.form and 'password' in request.form and 'email' in request.form:
        ...
        if account:
            ...
            cursor.execute('INSERT INTO user (id, first_name, last_name, email, password) VALUES (NULL, %s, %s, %s, %s)',
    ...
    if 'loggedin' in session:
        ...
        if request.method == 'POST' and 'name' in request.form and 'author' in request.form and 'publisher' in request.form and 'category' in request.form and 'rack' in request.form :
            ...
            if action == 'updateBook':
                ...
                cursor.execute('INSERT INTO book (`name`, `status`, `isbn`, `no_of_copy`, `categoryid`, `authorid`, `rackid`, `publisherid`, `picture`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)',
               (bookName, status, isbn, no_of_copy, category, author, rack, publisher, 'default_picture.jpg'))
                mysql.connection.commit()           
...
# Add this before the app.run() block to handle database connection and cleanup
@app.before_request
def before_request():
    g.connection = mysql.connection
    g.cursor = g.connection.cursor(MySQLdb.cursors.DictCursor)
